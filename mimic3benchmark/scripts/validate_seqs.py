...
def main(tables):
    ...
    for table in tables:
        print ("validatig results for table: " + table)

        n_reading = 0  # total number of readings for table
        ...
        for (index, subject) in enumerate(subjects):
            ...
            if not os.path.exists(os.path.join(args.subjects_root_path, subject, table.lower() + '.csv')):
                continue
            seq_table_df = pd.read_csv(os.path.join(args.subjects_root_path, subject, table.lower() + '.csv'), index_col=False,
                                    dtype={'HADM_ID': str, "ICUSTAY_ID": str})
            seq_table_df.columns = seq_table_df.columns.str.upper()
            n_reading += seq_table_df.shape[0]

            # we drop all events for them HADM_ID is empty
            # TODO: maybe we can recover HADM_ID by looking at ICUSTAY_ID
            empty_hadm += seq_table_df['HADM_ID'].isnull().sum()
            seq_table_df = seq_table_df.dropna(subset=['HADM_ID'])

            merged_df = seq_table_df.merge(stays_df, left_on=['HADM_ID'], right_on=['HADM_ID'],
            ...
            # if ICUSTAY_ID is empty in events.csv, we try to recover it
            ...
            merged_df = merged_df.dropna(subset = ['CHARTTIME'])
            ...
            to_write = merged_df[['SUBJECT_ID', 'HADM_ID', 'ICUSTAY_ID', 'CHARTTIME', 'ITEMID', 'FLAG']]
            to_write.to_csv(os.path.join(args.subjects_root_path, subject, table.lower() + '.csv'), index=False)

        print('n_readings: {}'.format(n_reading))
...
if __name__ == "__main__":
    tables = ['LABEVENTS', 'MICROBIOLOGYEVENTS']
    main(tables)
